          ...
          <div className="space-y-6 rounded border p-4" key={tier._id}>
            <div className="space-y-3">
              <dt className="h3 flex flex-wrap items-center gap-x-4">
                ...
                <Pretitle className="ml-auto text-xs">{tier.highlight}</Pretitle>
              </dt>

              <dt className="flex flex-wrap items-end gap-x-2">
                {!isNaN(tier.price.base) && <b className="h1">{formatPrice(tier.price.base)}</b>}
                {tier.price.suffix && <span className={cn(isNaN(tier.price.base) && 'h1')}>{tier.price.suffix}</span>}
                {tier.price.strikethrough && <s className="font-bold decoration-red-500">{formatPrice(tier.price.strikethrough)}</s>}
              </dt>
            </div>
