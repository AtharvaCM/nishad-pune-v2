...
const DeadLinksPlugin = ({ config, ...props }: Props & { config: Config }) => {
  ...
  const [linkResults, setLinkResults] = useState<LinkResults>(getDocumentLinks(contentBlocks as Modules));
          ...
          >
            ...
            >
              ...
              >
                ...
                {status !== 'success' && message && <p style={{ display: 'inline-block', margin: 0 }}>{message}</p>}
              </div>
            </div>
          </li>
        ))}
...
export const DeadLinks = (config: Config = defaultConfig): PreflightPlugin => {
  const WithConfigWrapper = (props: Props) => <DeadLinksPlugin config={config} {...props} />;
